...
def create_user():
    ...
    #Registra un cambio en la db(github)
    db.session.commit()

    return jsonify({"msg":"Usuario registrado con exito", "user":new_user.to_dict()}),200
...
def login():
    ...
    return jsonify({"msg":"Algo salio mal al intentat iniciar sesion, revise sus claves"}),400
...
def update(user_id):
    ...
    return jsonify({"msg":"Algo salio mal al intentar actualizar el usuario"})
